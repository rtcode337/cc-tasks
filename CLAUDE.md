...
- **プロジェクト未設定のタスクは「未分類」グループとして一番下**に出す(`lib/groups.ts` の
  ...
  該当が 0 件ならグループごと出さない。`ProjectGroups` はこれを `pinnedGroups` として
...
- **カードを別のプロジェクトのグループまで運ぶと、そのプロジェクトへ移る**
  ...
  - **逆(プロジェクト → 未分類)はできない**。PATCH の `projectId: null` は「変更しない」の意味で、
    紐づけを外す口が API に無いため。`moveTaskToProject` は未分類ゾーンなら false を返し、
    呼び出し側は元グループ内の並び替えとして続ける
...
- トップにはアーカイブ済みを出さない。代わりに一覧の一番下の「アーカイブしたプロジェクト →」から
  `/archived`(`ArchivedProjectsView`)へ。**この導線が唯一のアーカイブを戻す口なので消してはいけない**。
