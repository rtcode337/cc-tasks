...
- **ルール**: 全 Claude Code 環境に効かせたい決まりごとを Markdown で複数管理。「まとめて表示」で 1 本に連結してコピーできる(長押しドラッグの並び順がそのまま連結順)。規約リポジトリを設定すると ✳ で開くセッションに常に含まれる(→ ルールを Claude Code に効かせる)
...
- 「規約リポジトリ(下記)自体は配布専用なので自動では更新しない。更新はユーザーが
  Markdown か cc-tasks サーバーの URL を明示的に渡したときだけ」というルール
