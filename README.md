...
- **トップ**: タスク入力(数行)+ プロジェクト選択(任意)+ 保存。その下に未着手一覧をプロジェクトごとに折りたたみ表示(デフォルトは折りたたみ。未紐づけタスクは見出しなしで常に先頭に展開、続いてプロジェクトの並び順。グループ内は作成日時降順。開閉状態はブラウザに保存)
- **タスク一覧**: 未完了を作成日時降順で表示。「完了したタスクを表示」で完了分に切替(10 件ずつページング)。あとからプロジェクト紐づけも可
- **プロジェクト**: 作成・編集・アーカイブ・並び替え(☰ ハンドルをドラッグ。トップのグループ順に反映)
...
各カード右上の 📋 アイコンで本文をクリップボードへコピー。カードの「編集」で編集画面へ、「完了」で done にする。
物理削除は誤タップを避けるため一覧・詳細には置かず、タスク編集画面の「タスクを削除」から行う。
...
スマホではユニバーサルリンクで Claude アプリが開き、アプリはクエリのプリフィルを
引き継がないため内容は失われる(中継ページ経由の JS 遷移でも回避できなかった)。
スマホでは 📋 コピーで貼り付ける運用にする。
...
- **✳ ハンドオフをスマホでも効かせる**: Claude モバイルアプリはユニバーサルリンクの
  クエリ(`prompt` / `repositories`)を引き継がない。空タブ経由・中継ページ経由の
  JS 遷移でもユニバーサルリンクの発火は回避できなかったため、現状は直リンクとし、
  スマホは 📋 コピーで運用している。アプリがプリフィルを引き継ぐようになれば
  そのまま解決する
