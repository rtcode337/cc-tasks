...
- **トップ**: タスク入力(数行)+ プロジェクト選択(任意)+ 保存。その下に未着手一覧(作成日時降順)
- **タスク一覧**: 未完了を作成日時降順で表示。「完了したタスクを表示」で完了分に切替(10 件ずつページング)。あとからプロジェクト紐づけも可
- **プロジェクト**: 作成・編集・アーカイブ
- ヘッダ: トップ / タスク一覧 / プロジェクト / ログアウト / テーマ切替(明暗)
...
`https://claude.ai/code?prompt=…&repositories=…` を新規タブで開く(PC は Web 版 Claude Code、
スマホはユニバーサルリンクで Claude アプリが開く)。`repositories` にはプロジェクトの
