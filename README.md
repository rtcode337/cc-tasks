...
| タスクを持ち出す・戻す | トップ下部の**バックアップ**から「書き出し」/「読み込み」(→ [タスクのバックアップ](#タスクのバックアップ)) |

画面ごとの詳しい挙動・制約は [docs/詳細設計.md](docs/詳細設計.md) にまとめてある。

## タスクのバックアップ

トップ下部の**バックアップ**から、未完了タスクを**プロジェクト名とリポジトリ付きで
JSON に書き出せる**。書き出した内容はそのまま「読み込み」に貼って戻せるので、
テキストで保存しておけばバックアップになり、別の環境への引っ越しにも使える。

- プロジェクトは**名前で照合し、無ければリポジトリごと作る**。既にあるものは触らない
